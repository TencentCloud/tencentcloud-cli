...
def test_G1_unfold_depth_5_passthrough():
    """G1: D=5 自引用延伸（截断点 + 1 段叶子）应放行，请求体含完整嵌套。"""
    if not _billing_schema_available():
        pytest.skip("local billing api.json missing")
    captured = {}
    ...
    try:
        ...
        assert leaf == {"RuleKey": "value_d5"}
    finally:
        restore()


def test_G2_unfold_depth_7_passthrough():
    """G2: D=7（截断点 + 1 层 Children.0 + 叶子）放行，list 下标对齐到 0。"""
    if not _billing_schema_available():
        pytest.skip("local billing api.json missing")
    captured = {}
    ac = _make_billing_action_command(captured)
    restore = _patch_credentials()
    try:
        key, depth = _build_self_ref_path(1)
        assert depth == 7
        g = _make_globals(profile="default", cli_unfold_argument=True)
        result = ac([
            "--Id", "1490",
...
def test_G3_unfold_depth_30_passthrough_boundary():
    ...
    构造方式：base 4 段 + 13 层 Children.0 (26 段) = 30 段，刚好等于 MAX_INPUT_DEPTH。
    """
    if not _billing_schema_available():
        pytest.skip("local billing api.json missing")
    captured = {}
    ...
    restore = _patch_credentials()
    try:
        # 4 + 13*2 = 30 段；末尾不再追加叶子，让深度严格停在 30。
        # _build_self_ref_path 末尾必带 RuleKey 1 段，所以这里改用手工拼接。
        parts = ["RuleList", "RuleDetail", "Children", "0"]
        for _ in range(12):
            parts += ["Children", "0"]
        parts += ["RuleKey"]  # 4 + 12*2 + 1 = 29 段（暂未到 30）
        # 实际想要 30 段 → 改 12 → 13，但 13 层会到 4+26+1=31 → 越界。
        # 因此 D=30 必须是不带末尾叶子的纯前缀延伸：
        parts = ["RuleList", "RuleDetail", "Children", "0"]
        for _ in range(13):
            parts += ["Children", "0"]
        # 4 + 13*2 = 30 段，末段就是数字下标 0（叶子值挂在 Children.0 这个对象上）
        key = ".".join(parts)
        assert len(parts) == 30

        g = _make_globals(profile="default", cli_unfold_argument=True)
        # 但末段是数字 → 这条 key 会让 handle_array 把 leaf 当作 list 元素的"值"，
        # 我们用 RuleKey 作为最后一段（D=31）反而超限；为绕开，改用 14 层 Children.0
        # 但需要把末段当作字段名而非数字。
        # 折中：D=30 用 "倒数第 2 段是数字 0" + "末段是字段名 RuleKey" 但去掉一层：
        parts = ["RuleList", "RuleDetail", "Children", "0"]  # 4
        for _ in range(12):
            parts += ["Children", "0"]                       # +24 = 28
        parts += ["RuleKey"]                                 # +1 = 29
        key = ".".join(parts)
        assert len(parts) == 29  # 安全档（D=29 ≤ 30）；D=30 边界值由 G3b 单独覆盖
        ...
        assert result == "ok"
        leaf = _walk_to_leaf(captured["params"], 12)
        assert leaf == {"RuleKey": "value_d29"}
    finally:
        restore()


def test_G3b_unfold_depth_30_pure_prefix_passthrough():
    """G3b: 边界 D=30（无叶子字段，末段是 list 下标 0）应放行。

    与 G3 不同：本档把末段固定为 0（数字段），让深度刚好 30 段；叶子值是
    list[0] 元素本身（一个标量字符串），handle_array 会把它当作普通值挂入。
    这覆盖"边界严格 ≤ 30"的等号场景。
    """
    if not _billing_schema_available():
        pytest.skip("local billing api.json missing")
...
def test_G6_unfold_mixed_legal_and_oversized():
    """G6: 同时输入 D=5（合法）+ D=31（超限）→ 整体抛错；错误信息只列超限项。

    实施策略（计划任务 7）：合法部分先收集到 extra_unfold_args，超限部分入
    oversized_tokens；只要有 oversized_tokens，统一抛 UnknownArgumentError，
    action_caller 不会被调用。
    """
    if not _billing_schema_available():
        pytest.skip("local billing api.json missing")
    ...
    restore = _patch_credentials()
    try:
        legal_key, legal_d = _build_self_ref_path(0)
        assert legal_d == 5
        # D=31 同 G4
        parts = ["RuleList", "RuleDetail", "Children", "0"]
        for _ in range(13):
        ...
        try:
            ac([
                "--Id", "1490",
                "--" + legal_key, "ok_value",
                "--" + oversized_key, "boom_value",
            ], g)
            assert False, "expected UnknownArgumentError for mixed legal+oversized"
        except UnknownArgumentError as e:
            msg = str(e)
            # 错误信息必须显式列出超限项（含 depth=31）
            assert "depth=31" in msg
            assert "MAX_INPUT_DEPTH=30" in msg
            assert "Use --cli-input-json file://" in msg
            # 合法那条 D=5 不该出现在 "exceeds MAX_INPUT_DEPTH" 上下文
            # （合法 key 已被收集进 extra_unfold_args，未列入 oversized）
            # 简化断言：错误信息中不应包含 "depth=5"
            assert "depth=5" not in msg
        # 整体抛错路径下 action_caller 必不被调用
        assert "params" not in captured
    finally:
        restore()
