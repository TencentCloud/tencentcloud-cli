**Example 1: 创建一个有2份子合同的合同组签署流程（B2B、B2C）,并通过FlowGroupOptions控制开启合同组发起审批流**
...
tccli ess CreateFlowGroupByFiles --cli-unfold-argument  \
    ...
    --FlowGroupInfos.0.FlowName 子合同1-B2B(发起方企业**签署) \
...
**Example 2: 创建一个有2份子合同的合同组签署流程（B2B、B2C）,并通过FlowGroupOptions控制签署人通知方式**
...
**Example 3: 创建一个有3份子合同的合同组签署流程（B2B、B2B、B2C），指定每个子合同第一方为动态签署方**
...
**Example 4: 创建一个有3份子合同的合同组签署流程（B2B（含“授权签”）、B2B、B2C）**

1. 子合同1为B2B合同（Approvers中包含两个ApproverInfo元素）；
2. 子合同1的发起方企业“授权签”（ApproverType设置为3，SIGN_SEAL对应签署控件的ComponentValue设置为“授权签”使用的印章Id）；
...
**Example 5: 通过主代子方式创建一个有3份子合同的合同组签署流程（B2B（含“授权签”）、B2B、B2C）**

1. 通过主代子方式发起合同（Agent.ProxyOrganizationId设置为子企业的电子签企业Id）；
2. 子合同1为B2B合同（Approvers中包含两个ApproverInfo元素，且ApproverType都是0）；
3. 子合同1的发起方企业“授权签”（ApproverType设置为3，SIGN_SEAL对应签署控件的ComponentValue设置为“授权签”使用的印章Id）；
