...
from tencentcloud.rce.v20260130 import rce_client as rce_client_v20260130
from tencentcloud.rce.v20260130 import models as models_v20260130
...
def doDeleteNameListData(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.DeleteNameListData(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doCreateNameList(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.CreateNameList(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doManageIPPortraitRisk(args, parsed_globals):
    ...
    start_time = time.time()
    while True:
        rsp = client.ManageIPPortraitRisk(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doDescribeNameListDetail(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.DescribeNameListDetail(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doReportEvent(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    client = mod.RceClient(cred, g_param[OptionsDefine.Region], profile)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.ReportEventRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
        rsp = client.ReportEvent(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def parse_global_arg(parsed_globals):
    ...
    try:
        if g_param[OptionsDefine.ServiceVersion]:
            ...
            version = conf["rce"][OptionsDefine.Version]
            g_param[OptionsDefine.Version] = "v" + version.replace('-', '')

        if g_param[OptionsDefine.Endpoint] is None:
            g_param[OptionsDefine.Endpoint] = conf["rce"][OptionsDefine.Endpoint]
