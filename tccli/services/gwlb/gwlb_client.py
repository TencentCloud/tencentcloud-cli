...
from tencentcloud.gwlb.v20240906 import gwlb_client as gwlb_client_v20240906
from tencentcloud.gwlb.v20240906 import models as models_v20240906
...
def doDescribeGatewayLoadBalancers(args, parsed_globals):
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.DescribeGatewayLoadBalancersRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
        rsp = client.DescribeGatewayLoadBalancers(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doInquirePriceCreateGatewayLoadBalancer(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.InquirePriceCreateGatewayLoadBalancerRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
        rsp = client.InquirePriceCreateGatewayLoadBalancer(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def parse_global_arg(parsed_globals):
    ...
    try:
        if g_param[OptionsDefine.ServiceVersion]:
            ...
            version = conf["gwlb"][OptionsDefine.Version]
            g_param[OptionsDefine.Version] = "v" + version.replace('-', '')

        if g_param[OptionsDefine.Endpoint] is None:
            g_param[OptionsDefine.Endpoint] = conf["gwlb"][OptionsDefine.Endpoint]
