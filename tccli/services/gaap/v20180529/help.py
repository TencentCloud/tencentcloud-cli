...
INFO = {
  ...
  "DescribeRealServerStatistics": {
    "params": [
      ...
      {
        "name": "StartTime",
        "desc": "统计开始时间(2020-08-19 00:00:00)"
      },
      {
        "name": "EndTime",
        "desc": "统计结束时间(2020-08-19 23:59:59)"
      },
      {
        "name": "Granularity",
        "desc": "统计的数据粒度，单位：秒，仅支持1分钟-60和5分钟-300粒度"
      }
    ],
    "desc": "该接口（DescribeRealServerStatistics）用于查询源站健康检查结果的统计数据。源站状态展示位为1：正常或者0：异常。查询的源站需要在监听器或者规则上进行了绑定，查询时需指定绑定的监听器或者规则ID。该接口支持1分钟细粒度的源站状态统计数据展示。"
  },
