...
from tccli.services.sts import v20180813
from tccli.services.sts.v20180813 import help as v20180813_help


def doGetFederationToken(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetFederationToken", g_param[OptionsDefine.Version])
        return

    param = {
        "Name": argv.get("--Name"),
        "Policy": argv.get("--Policy"),
...
def doQueryApiKey(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("QueryApiKey", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.QueryApiKeyRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.QueryApiKey(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doAssumeRole(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("AssumeRole", g_param[OptionsDefine.Version])
        return
...
def sts_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "sts", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("sts", sts_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["sts"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["sts"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "sts", "desc": desc, "params": docstr}
    print(helpmsg)
