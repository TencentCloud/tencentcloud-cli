...
from tencentcloud.tia.v20180226 import tia_client as tia_client_v20180226
from tencentcloud.tia.v20180226 import models as models_v20180226
from tccli.services.tia import v20180226
from tccli.services.tia.v20180226 import help as v20180226_help
...
def doListJobs(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ListJobs", g_param[OptionsDefine.Version])
        return
...
def doCreateJob(argv, arglist):
    ...
    param = {
        ...
        "ScaleTier": argv.get("--ScaleTier"),
        "MasterType": argv.get("--MasterType"),
        "WorkerType": argv.get("--WorkerType"),
        "ParameterServerType": argv.get("--ParameterServerType"),
...
def doDescribeModel(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeModel", g_param[OptionsDefine.Version])
        return
...
def doListModels(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ListModels", g_param[OptionsDefine.Version])
        return
...
def doDeleteModel(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DeleteModel", g_param[OptionsDefine.Version])
        return
...
def doInstallAgent(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("InstallAgent", g_param[OptionsDefine.Version])
        return

    param = {
        "Cluster": argv.get("--Cluster"),
        "TiaVersion": argv.get("--TiaVersion"),
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.InstallAgentRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.InstallAgent(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
CLIENT_MAP = {
    "v20180226": tia_client_v20180226,

}

MODELS_MAP = {
    "v20180226": models_v20180226,
...
ACTION_MAP = {
    ...
    "QueryLogs": doQueryLogs,
    "ListModels": doListModels,
    "DeleteJob": doDeleteJob,
    "DeleteModel": doDeleteModel,
    "DescribeJob": doDescribeJob,
    "InstallAgent": doInstallAgent,
...
AVAILABLE_VERSIONS = {
     'v' + v20180226.version.replace('-', ''): {"help": v20180226_help.INFO,"desc": v20180226_help.DESC},

}


def tia_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "tia", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("tia", tia_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["tia"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["tia"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "tia", "desc": desc, "params": docstr}
    print(helpmsg)
