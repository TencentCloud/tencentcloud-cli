...
from tccli.services.domain import v20180808
from tccli.services.domain.v20180808 import help as v20180808_help


def doDescribeDomainPriceList(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeDomainPriceList", g_param[OptionsDefine.Version])
        return
...
def doCheckDomain(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CheckDomain", g_param[OptionsDefine.Version])
        return
...
def domain_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "domain", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("domain", domain_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["domain"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["domain"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "domain", "desc": desc, "params": docstr}
    print(helpmsg)
...
def get_actions_info():
    ...
    try:
        ...
        version = profile["domain"]["version"]
