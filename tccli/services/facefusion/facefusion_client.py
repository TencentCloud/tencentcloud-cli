...
def doDescribeMaterialList(args, parsed_globals):
    ...
    model = models.DescribeMaterialListRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
        rsp = client.DescribeMaterialList(model)
...
def doFaceFusion(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
...
def doFuseFace(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.FuseFaceRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
        rsp = client.FuseFace(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doFaceFusionLite(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.FaceFusionLiteRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
        rsp = client.FaceFusionLite(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
ACTION_MAP = {
    ...
    "FuseFace": doFuseFace,
    "FaceFusionLite": doFaceFusionLite,
...
def parse_global_arg(parsed_globals):
    ...
    try:
        if g_param[OptionsDefine.ServiceVersion]:
            ...
            version = conf["facefusion"][OptionsDefine.Version]
