...
from tencentcloud.drm.v20181115 import drm_client as drm_client_v20181115
from tencentcloud.drm.v20181115 import models as models_v20181115
from tccli.services.drm import v20181115
...
def doCreateEncryptKeys(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateEncryptKeys", g_param[OptionsDefine.Version])
        return

    param = {
        "DrmType": argv.get("--DrmType"),
        "Keys": Utils.try_to_json(argv, "--Keys"),
        "ContentId": argv.get("--ContentId"),
        "ContentType": argv.get("--ContentType"),
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.CreateEncryptKeysRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.CreateEncryptKeys(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doCreateLicense(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateLicense", g_param[OptionsDefine.Version])
        return

    param = {
        "DrmType": argv.get("--DrmType"),
        "LicenseRequest": argv.get("--LicenseRequest"),
        "ContentType": argv.get("--ContentType"),
...
def doDescribeKeys(argv, arglist):
    ...
    param = {
        ...
        "Tracks": Utils.try_to_json(argv, "--Tracks"),
        "ContentType": argv.get("--ContentType"),
        "RsaPublicKey": argv.get("--RsaPublicKey"),
        "ContentId": argv.get("--ContentId"),
...
def doDescribeAllKeys(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeAllKeys", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.DescribeAllKeysRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.DescribeAllKeys(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def drm_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "drm", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("drm", drm_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["drm"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["drm"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "drm", "desc": desc, "params": docstr}
    print(helpmsg)
