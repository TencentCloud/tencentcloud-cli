...
from tencentcloud.ump.v20200918 import ump_client as ump_client_v20200918
...
def doCreateCameraAlerts(args, parsed_globals):
    ...
    start_time = time.time()
    while True:
        rsp = client.CreateCameraAlerts(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doCreateMultiBizAlert(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.CreateMultiBizAlert(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doDeleteMultiBizAlert(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.DeleteMultiBizAlert(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doDescribeCameras(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
...
def doCreateProgramState(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.CreateProgramState(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doCreateServerState(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.CreateServerState(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doDescribeMultiBizBaseImage(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.DescribeMultiBizBaseImage(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doCreateCapture(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.CreateCapture(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
CLIENT_MAP = {
    "v20200918": ump_client_v20200918,

}

MODELS_MAP = {
    "v20200918": models_v20200918,
...
ACTION_MAP = {
    ...
    "CreateServerState": doCreateServerState,
    "DescribeMultiBizBaseImage": doDescribeMultiBizBaseImage,
    "DescribeZones": doDescribeZones,
    "CreateCapture": doCreateCapture,
...
def parse_global_arg(parsed_globals):
    ...
    try:
        if g_param[OptionsDefine.ServiceVersion]:
            ...
            version = conf["ump"][OptionsDefine.Version]
            g_param[OptionsDefine.Version] = "v" + version.replace('-', '')

        if g_param[OptionsDefine.Endpoint] is None:
            g_param[OptionsDefine.Endpoint] = conf["ump"][OptionsDefine.Endpoint]
