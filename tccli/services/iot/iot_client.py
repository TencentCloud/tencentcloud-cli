...
from tencentcloud.iot.v20180123 import iot_client as iot_client_v20180123
from tencentcloud.iot.v20180123 import models as models_v20180123
from tccli.services.iot import v20180123
from tccli.services.iot.v20180123 import help as v20180123_help


def doGetDataHistory(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetDataHistory", g_param[OptionsDefine.Version])
        return
    ...
    model = models.GetDataHistoryRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.GetDataHistory(model)
...
def doResetDevice(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ResetDevice", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.ResetDeviceRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.ResetDevice(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doIssueDeviceControl(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("IssueDeviceControl", g_param[OptionsDefine.Version])
        return
...
def doDeactivateRule(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DeactivateRule", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.DeactivateRuleRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.DeactivateRule(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doGetDevices(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetDevices", g_param[OptionsDefine.Version])
        return
...
def doAddTopic(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("AddTopic", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.AddTopicRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.AddTopic(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doGetProducts(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetProducts", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.GetProductsRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.GetProducts(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doAddRule(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("AddRule", g_param[OptionsDefine.Version])
        return
...
def doAddProduct(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("AddProduct", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.AddProductRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.AddProduct(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doPublishMsg(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("PublishMsg", g_param[OptionsDefine.Version])
        return

    param = {
        "Topic": Utils.try_to_json(argv, "--Topic"),
        "Message": Utils.try_to_json(argv, "--Message"),
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.PublishMsgRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.PublishMsg(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doUpdateRule(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("UpdateRule", g_param[OptionsDefine.Version])
        return
...
def doGetDeviceStatuses(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetDeviceStatuses", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.GetDeviceStatusesRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.GetDeviceStatuses(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doGetRules(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetRules", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.GetRulesRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.GetRules(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doGetTopic(argv, arglist):
    ...
    param = {
        "TopicId": Utils.try_to_json(argv, "--TopicId"),
        "ProductId": Utils.try_to_json(argv, "--ProductId"),
...
def doGetDevice(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetDevice", g_param[OptionsDefine.Version])
        return
...
def doGetDeviceData(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetDeviceData", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.GetDeviceDataRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.GetDeviceData(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doGetRule(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetRule", g_param[OptionsDefine.Version])
        return
...
def doGetDeviceLog(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetDeviceLog", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.GetDeviceLogRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.GetDeviceLog(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doGetTopics(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetTopics", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.GetTopicsRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.GetTopics(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doAddDevice(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("AddDevice", g_param[OptionsDefine.Version])
        return
...
def doGetProduct(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("GetProduct", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.GetProductRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.GetProduct(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
def doActivateRule(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ActivateRule", g_param[OptionsDefine.Version])
        return
...
AVAILABLE_VERSIONS = {
     'v' + v20180123.version.replace('-', ''): {"help": v20180123_help.INFO,"desc": v20180123_help.DESC},

}


def iot_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "iot", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("iot", iot_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["iot"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["iot"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "iot", "desc": desc, "params": docstr}
    print(helpmsg)
