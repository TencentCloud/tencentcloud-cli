...
from tencentcloud.bda.v20200324 import bda_client as bda_client_v20200324
from tencentcloud.bda.v20200324 import models as models_v20200324
from tccli.services.bda import v20200324
from tccli.services.bda.v20200324 import help as v20200324_help


def doSearchTrace(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("SearchTrace", g_param[OptionsDefine.Version])
        return
    ...
    param = {
        ...
        "TraceMatchThreshold": Utils.try_to_json(argv, "--TraceMatchThreshold"),
...
def doDetectBody(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DetectBody", g_param[OptionsDefine.Version])
        return
...
def doSegmentPortraitPic(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("SegmentPortraitPic", g_param[OptionsDefine.Version])
        return
...
def doCreateTrace(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateTrace", g_param[OptionsDefine.Version])
        return
...
def doCreateGroup(argv, arglist):
    ...
    param = {
        ...
        "GroupId": argv.get("--GroupId"),
        "Tag": argv.get("--Tag"),
        "BodyModelVersion": argv.get("--BodyModelVersion"),
...
def doSegmentCustomizedPortraitPic(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("SegmentCustomizedPortraitPic", g_param[OptionsDefine.Version])
        return

    param = {
        "SegmentationOptions": Utils.try_to_json(argv, "--SegmentationOptions"),
        "Image": argv.get("--Image"),
        "Url": argv.get("--Url"),
...
CLIENT_MAP = {
    "v20200324": bda_client_v20200324,
...
def bda_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "bda", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("bda", bda_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["bda"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["bda"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "bda", "desc": desc, "params": docstr}
    print(helpmsg)
