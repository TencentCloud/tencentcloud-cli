...
from tencentcloud.ags.v20250920 import ags_client as ags_client_v20250920
from tencentcloud.ags.v20250920 import models as models_v20250920

from jmespath import search
import time

def doCreatePreCacheImageTask(args, parsed_globals):
    ...
    model = models.CreatePreCacheImageTaskRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
        rsp = client.CreatePreCacheImageTask(model)
...
def doAcquireSandboxInstanceToken(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.AcquireSandboxInstanceTokenRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
        rsp = client.AcquireSandboxInstanceToken(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doUpdateSandboxTool(args, parsed_globals):
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.UpdateSandboxToolRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
...
def parse_global_arg(parsed_globals):
    ...
    try:
        if g_param[OptionsDefine.ServiceVersion]:
            ...
            version = conf["ags"][OptionsDefine.Version]
            g_param[OptionsDefine.Version] = "v" + version.replace('-', '')

        if g_param[OptionsDefine.Endpoint] is None:
            g_param[OptionsDefine.Endpoint] = conf["ags"][OptionsDefine.Endpoint]
