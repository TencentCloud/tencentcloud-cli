...
def doUpdatePlugins(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("UpdatePlugins", g_param[OptionsDefine.Version])
        return

    param = {
        "InstanceId": argv.get("--InstanceId"),
        "InstallPluginList": Utils.try_to_json(argv, "--InstallPluginList"),
        "RemovePluginList": Utils.try_to_json(argv, "--RemovePluginList"),
...
def doUpdateInstance(argv, arglist):
    ...
    param = {
        ...
        "ScaleType": Utils.try_to_json(argv, "--ScaleType"),
