...
from tccli.services.mvj import v20190926
from tccli.services.mvj.v20190926 import help as v20190926_help
...
AVAILABLE_VERSIONS = {
     'v' + v20190926.version.replace('-', ''): {"help": v20190926_help.INFO,"desc": v20190926_help.DESC},

}


def mvj_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "mvj", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("mvj", mvj_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["mvj"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["mvj"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "mvj", "desc": desc, "params": docstr}
    print(helpmsg)
