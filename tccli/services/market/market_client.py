...
from tencentcloud.market.v20191010 import market_client as market_client_v20191010
from tencentcloud.market.v20191010 import models as models_v20191010


def doSyncUserAndOrderInfo(args, parsed_globals):
    ...
    model = models.SyncUserAndOrderInfoRequest()
    model.from_json_string(json.dumps(args))
    rsp = client.SyncUserAndOrderInfo(model)
...
def doGetUsagePlanUsageAmount(args, parsed_globals):
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.GetUsagePlanUsageAmountRequest()
    model.from_json_string(json.dumps(args))
    rsp = client.GetUsagePlanUsageAmount(model)
    result = rsp.to_json_string()
    try:
        json_obj = json.loads(result)
    except TypeError as e:
...
def doFlowProductRemind(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.FlowProductRemindRequest()
    model.from_json_string(json.dumps(args))
    rsp = client.FlowProductRemind(model)
    result = rsp.to_json_string()
    try:
        json_obj = json.loads(result)
    except TypeError as e:
...
CLIENT_MAP = {
    "v20191010": market_client_v20191010,

}

MODELS_MAP = {
    "v20191010": models_v20191010,

}

ACTION_MAP = {
    "SyncUserAndOrderInfo": doSyncUserAndOrderInfo,
...
def parse_global_arg(parsed_globals):
    ...
    try:
        if g_param[OptionsDefine.ServiceVersion]:
            ...
            version = conf["market"][OptionsDefine.Version]
            g_param[OptionsDefine.Version] = "v" + version.replace('-', '')

        if g_param[OptionsDefine.Endpoint] is None:
            g_param[OptionsDefine.Endpoint] = conf["market"][OptionsDefine.Endpoint]
