...
from tencentcloud.zj.v20190121 import zj_client as zj_client_v20190121
from tencentcloud.zj.v20190121 import models as models_v20190121

from jmespath import search
import time

def doDelCrowdPack(args, parsed_globals):
...
def doDelTemplate(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
...
def doDescribeMmsInstanceList(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.DescribeMmsInstanceList(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doGetSmsCampaignStatus(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.GetSmsCampaignStatus(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doGetCrowdUploadInfo(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    start_time = time.time()
    while True:
        rsp = client.GetCrowdUploadInfo(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
CLIENT_MAP = {
    "v20190121": zj_client_v20190121,

}

MODELS_MAP = {
    "v20190121": models_v20190121,
...
def parse_global_arg(parsed_globals):
    ...
    try:
        if g_param[OptionsDefine.ServiceVersion]:
            ...
            version = conf["zj"][OptionsDefine.Version]
            g_param[OptionsDefine.Version] = "v" + version.replace('-', '')

        if g_param[OptionsDefine.Endpoint] is None:
            g_param[OptionsDefine.Endpoint] = conf["zj"][OptionsDefine.Endpoint]
