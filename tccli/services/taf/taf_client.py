...
def doDetectAccountActivity(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DetectAccountActivity", g_param[OptionsDefine.Version])
        return
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.DetectAccountActivityRequest()
    model.from_json_string(json.dumps(param))
    rsp = client.DetectAccountActivity(model)
    result = rsp.to_json_string()
    jsonobj = None
    try:
        jsonobj = json.loads(result)
...
ACTION_MAP = {
    ...
    "DetectAccountActivity": doDetectAccountActivity,
