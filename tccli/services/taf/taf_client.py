...
def doEnhanceTaDegree(args, parsed_globals):
...
def doRecognizeTargetAudience(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
...
def doRecognizeEffectiveFlow(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.RecognizeEffectiveFlowRequest()
    model.from_json_string(json.dumps(args))
    rsp = client.RecognizeEffectiveFlow(model)
    result = rsp.to_json_string()
    try:
        jsonobj = json.loads(result)
    except TypeError as e:
...
def doRecognizeCustomizedAudience(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
...
def doDetectFraudKOL(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
...
def doRecognizePreciseTargetAudience(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
...
def doSendTrafficSecuritySmsMessage(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
...
ACTION_MAP = {
    ...
    "RecognizeEffectiveFlow": doRecognizeEffectiveFlow,
...
def parse_global_arg(parsed_globals):
    ...
    try:
        if g_param[OptionsDefine.ServiceVersion]:
            ...
            version = conf["taf"][OptionsDefine.Version]
            g_param[OptionsDefine.Version] = "v" + version.replace('-', '')

        if g_param[OptionsDefine.Endpoint] is None:
            g_param[OptionsDefine.Endpoint] = conf["taf"][OptionsDefine.Endpoint]
