...
from tccli.services.cis import v20180408
from tccli.services.cis.v20180408 import help as v20180408_help


def doDescribeContainerInstances(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeContainerInstances", g_param[OptionsDefine.Version])
        return
...
def doCreateContainerInstance(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateContainerInstance", g_param[OptionsDefine.Version])
        return
...
def doInquiryPriceCreateCis(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("InquiryPriceCreateCis", g_param[OptionsDefine.Version])
        return
...
def doDescribeContainerInstance(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeContainerInstance", g_param[OptionsDefine.Version])
        return
...
def doDescribeContainerInstanceEvents(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeContainerInstanceEvents", g_param[OptionsDefine.Version])
        return
...
def doDeleteContainerInstance(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DeleteContainerInstance", g_param[OptionsDefine.Version])
        return
...
def doDescribeContainerLog(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeContainerLog", g_param[OptionsDefine.Version])
        return
...
def cis_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "cis", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("cis", cis_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["cis"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["cis"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "cis", "desc": desc, "params": docstr}
    print(helpmsg)
