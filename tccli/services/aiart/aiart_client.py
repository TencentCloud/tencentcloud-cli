...
from tencentcloud.aiart.v20221229 import aiart_client as aiart_client_v20221229
from tencentcloud.aiart.v20221229 import models as models_v20221229
...
def doSubmitTemplateToImageJob(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.SubmitTemplateToImageJobRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
        rsp = client.SubmitTemplateToImageJob(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
def doDescribeTemplateToImageJob(args, parsed_globals):
    g_param = parse_global_arg(parsed_globals)
    ...
    client._sdkVersion += ("_CLI_" + __version__)
    models = MODELS_MAP[g_param[OptionsDefine.Version]]
    model = models.DescribeTemplateToImageJobRequest()
    model.from_json_string(json.dumps(args))
    start_time = time.time()
    while True:
        rsp = client.DescribeTemplateToImageJob(model)
        result = rsp.to_json_string()
        try:
            json_obj = json.loads(result)
        except TypeError as e:
...
CLIENT_MAP = {
    "v20221229": aiart_client_v20221229,

}

MODELS_MAP = {
    "v20221229": models_v20221229,
...
ACTION_MAP = {
    ...
    "SubmitTemplateToImageJob": doSubmitTemplateToImageJob,
    ...
    "DescribeTemplateToImageJob": doDescribeTemplateToImageJob,
...
def parse_global_arg(parsed_globals):
    ...
    try:
        if g_param[OptionsDefine.ServiceVersion]:
            ...
            version = conf["aiart"][OptionsDefine.Version]
            g_param[OptionsDefine.Version] = "v" + version.replace('-', '')

        if g_param[OptionsDefine.Endpoint] is None:
            g_param[OptionsDefine.Endpoint] = conf["aiart"][OptionsDefine.Endpoint]
