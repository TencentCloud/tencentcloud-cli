...
from tccli.services.tcb import v20180608
from tccli.services.tcb.v20180608 import help as v20180608_help


def doCreateHostingDomain(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateHostingDomain", g_param[OptionsDefine.Version])
        return
...
def doDescribeEndUsers(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeEndUsers", g_param[OptionsDefine.Version])
        return
...
def doCreateAuthDomain(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateAuthDomain", g_param[OptionsDefine.Version])
        return
...
def doDescribeAuthDomains(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeAuthDomains", g_param[OptionsDefine.Version])
        return
...
def doCreatePostpayPackage(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreatePostpayPackage", g_param[OptionsDefine.Version])
        return
...
def doReinstateEnv(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ReinstateEnv", g_param[OptionsDefine.Version])
        return
...
def doModifyEndUser(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ModifyEndUser", g_param[OptionsDefine.Version])
        return
...
def doDescribePostpayPackageFreeQuotas(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribePostpayPackageFreeQuotas", g_param[OptionsDefine.Version])
        return

    param = {
        "EnvId": argv.get("--EnvId"),
        "FreeQuotaType": argv.get("--FreeQuotaType"),
...
def doCommonServiceAPI(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CommonServiceAPI", g_param[OptionsDefine.Version])
        return

    param = {
        "Service": argv.get("--Service"),
        "JSONData": argv.get("--JSONData"),
...
def doCheckTcbService(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CheckTcbService", g_param[OptionsDefine.Version])
        return
...
def doDeleteEndUser(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DeleteEndUser", g_param[OptionsDefine.Version])
        return

    param = {
        "EnvId": argv.get("--EnvId"),
        "UserList": Utils.try_to_json(argv, "--UserList"),
...
def doDescribeQuotaData(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeQuotaData", g_param[OptionsDefine.Version])
        return
...
def doModifyEnv(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ModifyEnv", g_param[OptionsDefine.Version])
        return
...
def doDescribeDatabaseACL(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeDatabaseACL", g_param[OptionsDefine.Version])
        return
...
def doDestroyEnv(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DestroyEnv", g_param[OptionsDefine.Version])
        return
...
def doDescribeEnvs(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeEnvs", g_param[OptionsDefine.Version])
        return
...
def doDestroyStaticStore(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DestroyStaticStore", g_param[OptionsDefine.Version])
        return

    param = {
        "EnvId": argv.get("--EnvId"),
        "CdnDomain": argv.get("--CdnDomain"),
...
def doModifyDatabaseACL(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ModifyDatabaseACL", g_param[OptionsDefine.Version])
        return
...
def doCreateStaticStore(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateStaticStore", g_param[OptionsDefine.Version])
        return
...
def doDescribeEnvLimit(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeEnvLimit", g_param[OptionsDefine.Version])
        return
...
def doDescribeEnvFreeQuota(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeEnvFreeQuota", g_param[OptionsDefine.Version])
        return

    param = {
        "EnvId": argv.get("--EnvId"),
        "ResourceTypes": Utils.try_to_json(argv, "--ResourceTypes"),
...
AVAILABLE_VERSIONS = {
     'v' + v20180608.version.replace('-', ''): {"help": v20180608_help.INFO,"desc": v20180608_help.DESC},

}


def tcb_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "tcb", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("tcb", tcb_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["tcb"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["tcb"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "tcb", "desc": desc, "params": docstr}
    print(helpmsg)
