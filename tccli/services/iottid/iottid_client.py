...
def doUploadDeviceUniqueCode(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("UploadDeviceUniqueCode", g_param[OptionsDefine.Version])
        return

    param = {
        "CodeSet": Utils.try_to_json(argv, "--CodeSet"),
...
def doDescribeAvailableLibCount(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeAvailableLibCount", g_param[OptionsDefine.Version])
        return
