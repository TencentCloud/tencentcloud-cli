...
INFO = {
  ...
  "SentenceSimilarity": {
    "params": [
      {
        "name": "SrcText",
        "desc": "计算相似度的源句子（仅支持UTF-8格式，不超过500字符）"
      },
      {
        "name": "TargetText",
        "desc": "计算相似度的目标句子（仅支持UTF-8格式，不超过500字符）"
      }
    ],
    "desc": "文本相似度接口能够基于深度学习技术来计算两个输入文本的相似度，相似度数值越大的两个文本在语义上越相似。目前仅支持短文本（不超过500字符）的相似度计算，长文本的相似度计算也即将推出。\n\n鉴于文本相似度是一个应用非常广泛的功能，腾讯知文自然语言处理团队在深度神经网络模型的基础上，专门针对文本相似任务进行了优化，并持续迭代更新。基于文本相似度，可以轻松实现诸如文本去重、相似推荐等功能。"
  },
  "DescribeRelation": {
    "params": [
      {
