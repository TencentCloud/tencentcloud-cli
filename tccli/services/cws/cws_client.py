...
from tccli.services.cws import v20180312
...
def doModifyMonitorAttribute(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ModifyMonitorAttribute", g_param[OptionsDefine.Version])
        return
...
def doCreateSitesScans(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateSitesScans", g_param[OptionsDefine.Version])
        return
...
def doCreateSites(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateSites", g_param[OptionsDefine.Version])
        return
...
def doCreateVulsMisinformation(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateVulsMisinformation", g_param[OptionsDefine.Version])
        return

    param = {
        "VulIds": Utils.try_to_json(argv, "--VulIds"),
...
def doDescribeSites(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeSites", g_param[OptionsDefine.Version])
        return
...
def doDescribeSitesVerification(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeSitesVerification", g_param[OptionsDefine.Version])
        return
...
def doModifyConfigAttribute(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ModifyConfigAttribute", g_param[OptionsDefine.Version])
        return

    param = {
        "NoticeLevel": argv.get("--NoticeLevel"),
...
def doVerifySites(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("VerifySites", g_param[OptionsDefine.Version])
        return
...
def doDeleteMonitors(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DeleteMonitors", g_param[OptionsDefine.Version])
        return

    param = {
        "MonitorIds": Utils.try_to_json(argv, "--MonitorIds"),
...
def doDeleteSites(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DeleteSites", g_param[OptionsDefine.Version])
        return
...
def doDescribeMonitors(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeMonitors", g_param[OptionsDefine.Version])
        return
...
def doDescribeSiteQuota(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeSiteQuota", g_param[OptionsDefine.Version])
        return
...
AVAILABLE_VERSIONS = {
     'v' + v20180312.version.replace('-', ''): {"help": v20180312_help.INFO,"desc": v20180312_help.DESC},

}


def cws_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "cws", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("cws", cws_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["cws"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["cws"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "cws", "desc": desc, "params": docstr}
    print(helpmsg)
