...
from .utils import init_cos_client, match_filters, build_cos_key, TransferProgressMonitor
    ...
    try:
        if recursive:
            _move_by_prefix(client, bucket, cos_key, dest_bucket, dest_key,
                            region, dest_region, storage_class, include, exclude, routines)
        else:
            _move_single(client, bucket, cos_key, dest_bucket, dest_key,
                         region, storage_class)
...
def _move_by_prefix(client, bucket, prefix, dest_bucket, dest_prefix,
                    src_region, dest_region, storage_class, include, exclude, routines):
    """递归移动指定前缀下的所有对象
